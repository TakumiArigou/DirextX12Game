...
*/
	...
	//ランバートパイプライン生成
	{
		...
		m_PipelineState["Lanbert"] =
			CreatePipeline(
				"Shader\\LanbertVS.cso",
				"Shader\\LanbertPS.cso",
				RTVFormats, _countof(RTVFormats));
	}
	...
	//マテリアルバッファ生成
	{
		m_MaterialBuffer = CreateRenderTarget(
			m_BackBufferWidth,
			m_BackBufferHeight,
			DXGI_FORMAT_R16G16B16A16_FLOAT
		);
...
void RenderManager::DrawBegin()
{
	...
	m_GraphicsCommandList->ClearRenderTargetView(m_ColorBuffer->RTVHandle, materialStatus.m_ClearColor, 0, nullptr);
...
void RenderManager::DrawEnd()
{
	...
	//レンダーターゲットをポストエフェクトバッファに変更
	{
		...
		//レンダーターゲットの設定
		m_GraphicsCommandList->OMSetRenderTargets(1, &m_PostEffectBuffer->RTVHandle, TRUE, &m_DepthBufferHandle);
	...
	//デファードパス
	{
		...
		//マテリアル設定
		{
			...
			SetConstant(RenderManager::CONSTANT_TYPE::SUBSET, &material, sizeof(material));
		}
