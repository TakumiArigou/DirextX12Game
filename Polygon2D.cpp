...
Polygon2D::Polygon2D()
{
	RenderManager* renderManeger = RenderManager::GetInstance();

	//テクスチャ読み込み
	m_Texture = renderManeger->LoadTexture("Asset\\field004.dds");

	m_VertexBuffer = renderManeger->CreateVertexBuffer(sizeof(VERTEX_3D), 4);
...
void Polygon2D::Draw()
{
...

	{
		...
		projection = XMMatrixOrthographicOffCenterLH(0.0f, 
			...
			renderManager->GetBackBufferHeight(), 0.0f, 0.0f, 1.0f);
